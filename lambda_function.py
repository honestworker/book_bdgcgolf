...
def login_function(memno, password):
    ...
    if hasattr(res, 'cookies'):
        result['is_setcookie'] = 0
        result['cookie'] = res.cookies
    else:
...
# Search And Book Function
def get_book_eventid(cookie_info, next_book_date, comments):
    bdgc_eventlist_url = "https://www.bdgc.com.au/views/members/booking/eventList.xhtml"
    if cookie_info['is_setcookie']:
        bdgc_header_data = {
        ...
        res = requests.get(url=bdgc_eventlist_url, headers=bdgc_header_data, cookies=cookie_info['cookie'])
    
    #print("get_book_eventid:\n");
    #print(res.text);
...
def get_book_rowid(cookie_info, book_eventid):
    ...
    if cookie_info['is_setcookie']:
        ...
        res = requests.get(url=bdgc_book_url, headers=bdgc_header_data, cookies=cookie_info['cookie'])
    
    #print("get_book_rowid 1:\n");
    #print(res.text);
    
    soup = bs(res.text, 'html.parser')
    book_available_groups = soup.find_all('div', attrs={'class': 'available'})
    if book_available_groups:
        for book_group in book_available_groups:
            ...
            if len(taken_cells) == 0:
                ...
                if cookie_info['is_setcookie']:
                    ...
                    block_res = requests.get(url=bdgc_block_book_url, headers=bdgc_header_data, cookies=cookie_info['cookie'])
                
                #print("get_book_rowid 2:\n");
                #print(block_res.text);
...
def book_golf(cookie_info, book_eventid, book_rowid):
    ...
    if cookie_info['is_setcookie']:
        ...
        res = requests.get(url=bdgc_book_form_url, headers=bdgc_header_data, cookies=cookie_info['cookie'])
    ...
    if cookie_info['is_setcookie']:
        ...
        auto_book_res = requests.get(url=bdgc_auto_book_url, headers=bdgc_header_data, cookies=cookie_info['cookie'])
    ...
    if auto_book_res.status_code == 200:
        ...
        if error_contain:
            ...
            if cookie_info['is_setcookie']:
                ...
                book_fix_res = requests.get(url=book_fix_url, headers=bdgc_header_data, cookies=cookie_info['cookie'])
            ...
            if book_fix_confirm_button:
                ...
                if book_fix_book_form_selects:
                    ...
                    if cookie_info['is_setcookie']:
                        book_confirm_res = requests.post(url=bdgc_confirm_booking_url, headers=bdgc_header_data, data=book_fix_form_data, )
                    else:
                        book_confirm_res = requests.post(url=bdgc_confirm_booking_url, headers=bdgc_header_data, data=book_fix_form_data, cookies=cookie_info['cookie'])
...
def book_handler(event, context):
    ...
    next_book_date1 = date_after_eight_days.strftime('%a %d %b')

    if now_second:
        time.sleep(now_second)
    
    book_eventid = get_book_eventid(cookie_info, next_book_date, event['comments'])
    
    #print("Event ID" + book_eventid)
    loop_count = 0
    while book_eventid == '':
    ...
    if book_eventid:
        while book_row_result['row_id'] == '':
            ...
            #print("Row ID" + book_row_result['row_id'])
